...
function walkDir(dir) {
  ...
  files.forEach(file => {
    ...
    if (stat.isDirectory()) {
      ...
      // Check if file has quoted paths in image lines
      if (content.includes(`'`) && content.includes(`image:`)) {
        ...
        const newLines = lines.map((line, idx) => {
          // Match lines with "image:" that have path-like quotes
          if (line.includes('image:') && line.includes(`'`) && line.includes('/images/')) {
            modified = true;
            // Preserve indentation
            const indent = line.match(/^\s*/)[0];
            return `${indent}image: ${mappingEntry.newImagePath}`;
          }
          return line;
        });
