...
function processFiles(dir, collection) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      // For accompagnements: use "slug" field
      // For articles: use "seoSlug" field
      let slug;
      if (collection === 'services') {
        slug = yaml.slug || path.basename(filePath, '.mdoc');
      } else {
        slug = yaml.seoSlug || path.basename(filePath, '.mdoc');
      }
...
// Save mapping
fs.writeFileSync('migration-lang-mapping-corrected.json', JSON.stringify(allResults, null, 2), 'utf8');

console.log(`✅ Mapped ${allResults.length} files:`);
console.log(`   - ${accompagnements.length} accompagnements (services)`);
console.log(`   - ${articles.length} articles`);
console.log(`\n📄 Output: migration-lang-mapping-corrected.json`);
