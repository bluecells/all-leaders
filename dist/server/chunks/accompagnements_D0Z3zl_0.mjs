...
import 'piccolore';
import { m as maybeRenderHead, e as addAttribute, d as renderTemplate, c as renderComponent } from './server_DNeM8zG_.mjs';
...
const $$AccompagnementsGrid = createComponent(async ($$result, $$props, $$slots) => {
  ...
  function buildAccompagnementUrl(accompagnement, categorySlug) {
    const parts = accompagnement.id.split("/");
    const slug = parts[parts.length - 1]?.replace(/\.yaml$/, "") || "";
    console.log("Building URL for:", accompagnement.id, "slug:", slug, "categorySlug:", categorySlug);
