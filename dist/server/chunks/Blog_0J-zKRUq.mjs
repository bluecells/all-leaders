...
import 'piccolore';
import { d as renderTemplate, f as defineScriptVars, e as addAttribute, m as maybeRenderHead } from './server_D2WlyLFw.mjs';
import 'clsx';
import { g as getCollection, a as getEntry } from './_astro_content_DMBpWl0a.mjs';
...
const $$Blog = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const articlesData = sortedArticles.map((article) => {
    ...
    const permalink = validLang === "it" ? `/limologiche/${categorySlug || "uncategorized"}/${cleanSlug}` : `/${validLang}/limologiche/${categorySlug || "uncategorized"}/${cleanSlug}`;
...
#${localizedName} </button>`;
    ...
    return renderTemplate`<div class="article-card group"${addAttribute(categorySlug, "data-cat")}${addAttribute(JSON.stringify(rawTags), "data-tags")}${addAttribute(title.toLowerCase(), "data-title")} data-astro-cid-b4mpjmb3> <div class="relative aspect-16/10 mb-2 shadow-sm bg-gray-100 overflow-hidden" data-astro-cid-b4mpjmb3> <div class="absolute top-0 left-0 z-20" data-astro-cid-b4mpjmb3> <div class="bg-limolo-brand backdrop-blur-sm px-2 pb-1 rounded-md shadow-sm" data-astro-cid-b4mpjmb3> <span class="text-[10px] font-bold uppercase tracking-tighter text-white" data-astro-cid-b4mpjmb3> ${new Date(pubDate).toLocaleDateString(
      validLang === "it" ? "it-IT" : validLang === "fr" ? "fr-FR" : "en-US",
      { day: "2-digit", month: "short", year: "numeric" }
    )} </span> </div> </div> <a${addAttribute(permalink, "href")} data-astro-cid-b4mpjmb3> ${photoSrc ? renderTemplate`<img${addAttribute(photoSrc, "src")}${addAttribute(photoAlt, "alt")} class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 ease-out" data-astro-cid-b4mpjmb3>` : renderTemplate`<div class="w-full h-full bg-gray-200 flex items-center justify-center text-gray-500 text-sm" data-astro-cid-b4mpjmb3>
Pas d'image
</div>`} </a> </div> <div class="flex flex-col items-start px-3" data-astro-cid-b4mpjmb3> ${displayCategory !== "Uncategorized" && renderTemplate`<button class="filter-cat-link text-[8px] my-1 uppercase tracking-[0.2em] font-bold text-limolo-green hover:text-limolo-accent transition-colors"${addAttribute(displayCategory.toLowerCase(), "data-category")} data-astro-cid-b4mpjmb3> ${displayCategory} </button>`} <a${addAttribute(permalink, "href")} class="block group w-full" data-astro-cid-b4mpjmb3> <h3 class="text-lg my-2 font-serif text-limolo-green leading-snug group-hover:text-limolo-accent transition-colors" data-astro-cid-b4mpjmb3> ${title} </h3> </a> ${displayTags.length > 0 && renderTemplate`<div class="flex flex-wrap gap-2 mt-2" data-astro-cid-b4mpjmb3> ${displayTags.map((tag) => renderTemplate`<span class="text-[10px] text-gray-500" data-astro-cid-b4mpjmb3>#${tag}</span>`)} </div>`} <a${addAttribute(permalink, "href")} class="inline-flex items-center mt-4 text-[11px] font-bold uppercase tracking-widest text-limolo-earth group-hover:translate-x-2 transition-transform duration-300" data-astro-cid-b4mpjmb3> ${t.cta} <span class="ml-2 text-lg" data-astro-cid-b4mpjmb3>→</span> </a> </div> </div>`;
