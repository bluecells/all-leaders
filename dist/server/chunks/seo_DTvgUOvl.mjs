...
import 'piccolore';
import { c as renderComponent, d as renderTemplate, m as maybeRenderHead, e as addAttribute } from './server_GXSVGQot.mjs';
import { $ as $$Layout, r as renderScript } from './Layout_CBD6Fc-X.mjs';
import { g as getCollection } from './_astro_content_BLyoDwpZ.mjs';

const $$Seo = createComponent(async ($$result, $$props, $$slots) => {
  const [articles, pages, landingPages, rooms, faq] = await Promise.all([
  ...
  const allEntries = [
    ...
    if (entry.type === "Article") {
      ...
      finalPath = `${langPrefix}/limologiche/${category}/${cleanSlug}`;
    } else if (entry.type === "Room") {
      const segment = lang === "fr" ? "chambres" : lang === "en" ? "rooms" : "camere";
      finalPath = `${langPrefix}/${segment}/${cleanSlug}`;
    } else if (entry.type === "Page") {
      finalPath = cleanSlug === "home" ? `${langPrefix}/` : `${langPrefix}/${cleanSlug}`;
    } else if (entry.type === "FAQ") {
      finalPath = `${langPrefix}/${cleanSlug}`;
    }
