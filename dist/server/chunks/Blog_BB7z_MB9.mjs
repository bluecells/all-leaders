...
import 'piccolore';
import { d as renderTemplate, g as defineScriptVars, e as addAttribute, m as maybeRenderHead } from './server_Du_7o2g7.mjs';
import 'clsx';
import { g as getCollection, b as getEntry } from './_astro_content_C_UNwJv_.mjs';
...
const $$Blog = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate(_a || (_a = __template(["", "<div", '> <div class="flex flex-col justify-between lg:flex-row gap-4"> <aside class="lg:w-50 lg:order-2 space-y-10 lg:sticky lg:top-40 lg:self-start"> <div class="search-box mb-0 text-center "> <h4 class="text-[10px] md:text-left! uppercase tracking-[0.2em] font-bold text-gray-400 mb-2"> ', ' </h4> <input type="text" id="search-input"', ' class="w-full border-b border-gray-300 py-1 focus:border-limolo-green outline-none bg-transparent text-sm transition-colors text-center lg:text-left placeholder:text-center lg:placeholder:text-left"> </div> <div class="categories-box text-center lg:text-left"> <h4 class="text-[10px] md:text-left! uppercase tracking-[0.2em] font-bold text-gray-400 pt-4 mb-4"> ', ' </h4> <div class="flex flex-wrap gap-2 justify-center lg:justify-start"> <button class="filter-cat active category-cloud-item px-3 py-1 rounded-md text-[11px] font-semibold border-2 border-limolo-green text-limolo-green bg-limolo-green/10 transition-all hover:shadow-md" data-category="all"> ', " </button> ", ' </div> </div> <div class="tags-box"> <h4 class="text-[10px] md:text-left! uppercase tracking-[0.2em] font-bold text-gray-400 mb-4">Tags</h4> <div class="flex flex-wrap gap-2"> ', ' </div> </div> </aside> <main class="lg:flex-1 lg:order-1 max-w-full"> <div class="grid gap-4 grid-cols-1 lg:grid-cols-4" id="blog-grid"', "", "", "", "", "> ", ' </div> <div id="no-results" class="hidden text-center py-20 text-gray-400 font-serif italic text-xl"> ', " </div> ", ' </main> </div> </div> <!-- Template pour cloner les articles - gardé en DOM pour que Tailwind le voit --> <div id="article-template" style="display: none;"> <div class="article-card group flex flex-col border border-gray-200 rounded-lg hover:shadow-md transition-shadow overflow-hidden"> <div class="relative w-full aspect-16/10 shadow-sm bg-gray-100 overflow-hidden shrink-0"> <div class="absolute top-0 left-0 z-20"> <div class="bg-blue-950/50 h-5 leading-none backdrop-blur-sm px-4 py-0"> <span class="article-date text-[10px] font-bold uppercase tracking-tighter text-white"></span> </div> </div> <a href="" class="article-link block w-full h-full"> <img src="" alt="" class="article-image w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 ease-out"> </a> </div> <div class="flex flex-col h-full items-center justify-between flex-1 py-2"> <div class="w-full"> <button class="filter-cat-link article-category text-[10px] border-b-1 border-b-mist-200 mb-2 uppercase tracking-[0.2em] font-bold text-center! w-full text-limolo-green hover:text-limolo-accent transition-colors hidden"></button> <a href="" class="article-link block group w-full"> <h3 class="text-sm m-0 font-serif text-limolo-green leading-tight group-hover:text-limolo-accent transition-colors line-clamp-4 article-title"></h3> </a> </div> <a href="" class="article-link btn-limolo inline-flex px-8 rounded-sm py-0 items-center justify-center gap-2 mt-4 w-[90%]"> <span class="article-cta">Lire</span><span class="text-lg">→</span> </a> </div> </div> </div>  <script>(function(){', `
  ...
  function getFilteredArticles() {
    ...
    return allArticles.filter((article) => {
      const catMatch = activeCategory === 'all' || (article.categorySlug?.toLowerCase() || '') === activeCategory;
      const tagMatch = activeTags.length === 0 || activeTags.some((tag) => (article.rawTags || []).includes(tag));
  ...
  function createArticleCard(article) {
    ...
    if (imgEl && article.photoSrc) {
      ...
      noImageDiv.className = 'w-full h-full bg-gray-200 flex items-center justify-center text-gray-500 text-xs';
  ...
  function reattachEventListeners() {
    const newFilterCatLinks = document.querySelectorAll('.filter-cat-link:not([data-listener-attached])');
...
#${localizedName} </button>`;
    ...
    return renderTemplate`<div class="article-card group flex flex-col border border-gray-200 rounded-lg hover:shadow-md transition-shadow overflow-hidden"${addAttribute(categorySlug, "data-cat")}${addAttribute(JSON.stringify(rawTags), "data-tags")}${addAttribute(title.toLowerCase(), "data-title")}>  <div class="relative w-full aspect-16/10 shadow-sm bg-gray-100 overflow-hidden shrink-0"> <div class="absolute top-0 left-0 z-20"> <div class="bg-blue-950/50 h-5 leading-none backdrop-blur-sm px-4 py-0"> <span class="text-[10px] font-bold uppercase tracking-tighter text-white"> ${new Date(pubDate).toLocaleDateString(
      validLang === "it" ? "it-IT" : validLang === "fr" ? "fr-FR" : "en-US",
      { day: "2-digit", month: "short", year: "numeric" }
    )} </span> </div> </div> <a${addAttribute(permalink, "href")}> ${photoSrc ? renderTemplate`<img${addAttribute(photoSrc, "src")}${addAttribute(photoAlt, "alt")} class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 ease-out">` : renderTemplate`<div class="w-full h-full bg-gray-200 flex items-center justify-center text-gray-500 text-xs">
Pas d'image
</div>`} </a> </div>  <div class="flex flex-col h-full items-center justify-between flex-1 py-2"> <div class="w-full"> ${displayCategory !== "Uncategorized" && renderTemplate`<button class="filter-cat-link text-[10px] border-b-1 border-b-mist-200 mb-2 uppercase tracking-[0.2em] font-bold text-center! w-full text-limolo-green hover:text-limolo-accent transition-colors"${addAttribute(displayCategory.toLowerCase(), "data-category")}> ${displayCategory} </button>`} <a${addAttribute(permalink, "href")} class="block group w-full"> <h3 class="text-sm m-0 font-serif text-limolo-green leading-tight group-hover:text-limolo-accent transition-colors line-clamp-4"> ${title} </h3> </a> </div> <a${addAttribute(permalink, "href")} class="btn-limolo inline-flex px-8 py-0 items-center justify-center rounded-sm gap-2 mt-4 w-[90%]"> ${t.cta} <span class="text-lg">→</span> </a> </div> </div>`;
