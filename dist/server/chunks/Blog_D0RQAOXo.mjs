...
import 'piccolore';
import { d as renderTemplate, g as defineScriptVars, e as addAttribute, m as maybeRenderHead } from './server_01FeskOi.mjs';
import 'clsx';
import { g as getCollection, b as getEntry } from './_astro_content_CJljDULs.mjs';
...
Pas d'image
</div>`} </a> </div>  <div class="flex flex-col h-full items-center justify-between flex-1"> <div class="w-full"> ${displayCategory !== "Uncategorized" && renderTemplate`<button class="filter-cat-link text-[10px] border-b border-b-mist-200 py-1 mb-1 uppercase tracking-[0.2em] font-bold text-center! w-full text-limolo-green hover:text-limolo-accent transition-colors"${addAttribute(displayCategory.toLowerCase(), "data-category")}> ${displayCategory} </button>`} <div class="flex flex-col items-center justify-center min-h-14"> <a${addAttribute(permalink, "href")} class="block group w-full px-2"> <h3 class="text-xs font-light m-0 text-limolo-green leading-tight group-hover:text-limolo-accent transition-colors line-clamp-4"> ${title} </h3> </a> </div> </div> <a${addAttribute(permalink, "href")} class="border border-gray-200 bg-gray-100 hover:bg-gray-200 transition-colors inline-flex px-8 py-1 my-2 items-center justify-center rounded-sm gap-2 w-[90%]"> ${t.cta} <span class="text-md">→</span> </a> </div> </div>`;
