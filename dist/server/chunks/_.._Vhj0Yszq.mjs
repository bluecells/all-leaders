...
import 'piccolore';
import { c as renderComponent, d as renderTemplate, m as maybeRenderHead, e as addAttribute, g as renderSlot } from './server_GXSVGQot.mjs';
import { a as getEntry, g as getCollection, r as renderEntry } from './_astro_content_BLyoDwpZ.mjs';
import { $ as $$Layout, r as renderScript } from './Layout_CBD6Fc-X.mjs';
import 'clsx';
import { $ as $$ActionBar } from './ActionBar_h1PoSJ_W.mjs';
...
"${cleanText(excerpt)}"
    ...
    related.map(async (item) => {
      ...
      const url = lang === "it" ? `/limologiche/${item.data.category}/${cleanSlug}` : `/${lang}/limologiche/${item.data.category}/${cleanSlug}`;
...
function buildPageUrl(page, lang) {
...
function buildRoomUrl(room, lang) {
...
async function findAlternateUrls(entry, entryType, collections) {
  ...
  const collection = entryType === "page" ? collections.pages : entryType === "article" ? collections.articles : entryType === "room" ? collections.rooms : collections.faq;
  const buildUrl = entryType === "page" ? buildPageUrl : entryType === "article" ? (e, l) => buildArticleUrl(e, l, collections.categories) : entryType === "room" ? buildRoomUrl : (e, l) => buildFaqUrl(e, l, collections.categories);
...
const $$ = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const [articles, pages, rooms, faq, landingPages] = await Promise.all([
  ...
  let entry = null;
  let type = null;
  if (route.startsWith("limologiche/")) {
    const [, categorySlug, articleSlug] = route.split("/");
    const allCategories = await getCollection("category");
    entry = articles.find((a) => {
      const cleanSlug = (a.data.seoSlug || a.id.split("/").pop())?.replace(/^\/|\/$/g, "");
  ...
  if (!entry) {
    const roomSlug = route.split("/").pop();
    entry = rooms.find((r) => {
      const cleanSlug = (r.data.seoSlug || r.id.split("/").pop())?.replace(/^\/|\/$/g, "");
      if (r.data.lang !== lang) return false;
      if (lang === "fr" && route.startsWith("chambres/")) return cleanSlug === roomSlug;
      if (lang === "en" && route.startsWith("rooms/")) return cleanSlug === roomSlug;
      return false;
    });
  ...
  if (!entry) {
    entry = pages.find((p) => {
      const cleanSlug = (p.data.seoSlug || p.id)?.replace(/^\/|\/$/g, "");
      return p.data.lang === lang && cleanSlug === route;
    });
    type = entry ? "page" : null;
  }
  if (!entry) {
